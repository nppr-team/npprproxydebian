...
expect "Подсеть (например, 48):" { send -- "64\r" }
expect "Введите логин:" { send -- "mylogin\r" }
expect "Введите пароль:" { send -- "mypassword\r" }
expect "Введите номер (1, 2 или 3):" { send -- "1\r" }
expect "Введите номер (1 или 2):" { send -- "1\r" }
expect "Интервал ротации (в минутах, 0 для отключения, по умолчанию 0):" { send -- "0\r" }
expect "Количество прокси:" { send -- "100\r" }
expect "Введите номер (1 или 2):" { send -- "1\r" }
